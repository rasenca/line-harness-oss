...
## Update (2026-07-23) — Q-008(automation アクション数) のコード裏取り

P7 で実装を確認。

**自動化アクションは 8 種＝本 ADR 30 行目（表「8 種」）が正、docs の「6 種」概要は stale。**
- `apps/worker/src/services/event-bus.ts:239` からの `switch (action.type)` の case は **8 個**: `add_tag` / `remove_tag` / `start_scenario` / `send_message` / `send_webhook` / `switch_rich_menu` / `remove_rich_menu` / `set_metadata`。本 ADR 30 行目の列挙と完全一致。
- → 本 ADR の「7 種イベント × 8 種アクション」は**現行の正**。無限ループ検出のコードガードは引き続き未確認（doc どおり運用ルール委譲の可能性が高い）＝この点は継続留保。

→ [Q-008](../open-questions.md) の automation アクション数は ANSWERED。
